...
const finalMessageOverlay = document.querySelector(".final-message-overlay");
...
const finalMessageShowTime = 104.0; // 01:44
...
function updateSubtitleOverlay() {
  ...
  const timeline = (!musica.paused && Number.isFinite(musica.currentTime))
    ? musica.currentTime
    : video.currentTime;
  const time = Number.isFinite(timeline) ? timeline : 0;
...
function showFinalMessage() {
  ...
  document.body.classList.remove("lyric-focus");

  if (finalMessageOverlay) {
    finalMessageOverlay.classList.add("is-visible");
  }
}
...
async function startVideo() {
  ...
  try {
    const results = await Promise.allSettled([video.play(), musica.play()]);
    const videoStarted = results[0].status === "fulfilled";
    const musicStarted = results[1].status === "fulfilled";

    // Inicia la experiencia aunque solo uno arranque; evita pedir varios toques.
...
startBtn.addEventListener("pointerdown", startVideo, { passive: true });
